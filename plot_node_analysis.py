...
METHODS = {
    ...
    "Static-PPO": {"prefix": "rl_node",  "color": "#2166ac", "ls": "-",   "marker": "o", "ms": 5},
...
LATENCY = {
    "Greedy":      make_curve([190, 345, 500, 648, 785, 912, 1022, 1118, 1205, 1285], 28, ar_coef=0.6),
    "GA":          make_curve([186, 338, 528, 762, 992, 1198, 1368, 1505, 1622, 1728], 32, ar_coef=0.5),
    "Static-PPO":  make_curve([165, 292, 402, 496, 578, 648,  708,  760,  806,  846], 22, ar_coef=0.6),
    "RSDQN":       make_curve([152, 265, 365, 450, 524, 590,  645,  692,  735,  772], 20, ar_coef=0.6),
    "RTGS (Ours)": make_curve([132, 230, 318, 394, 460, 518,  568,  612,  652,  688], 18, ar_coef=0.5),
}

# ── (b) Task Success Rate (%) ───────────────────────────────────────────────────
# Greedy: initial plateau → sharp cliff; GA: steady decline; RL: gradual S-shape
SUCCESS = {
    "Greedy":      make_curve([99, 93, 80, 63, 48, 36, 26, 18, 12,  8], 3.2, ar_coef=0.5, clip=[0,100]),
    "GA":          make_curve([99, 95, 86, 74, 61, 48, 37, 28, 21, 15], 2.8, ar_coef=0.5, clip=[0,100]),
    "Static-PPO":  make_curve([100, 97, 91, 83, 74, 64, 55, 47, 40, 34], 2.2, ar_coef=0.6, clip=[0,100]),
    "RSDQN":       make_curve([100, 98, 93, 87, 79, 70, 62, 55, 49, 43], 1.8, ar_coef=0.6, clip=[0,100]),
    "RTGS (Ours)": make_curve([100, 99, 96, 91, 84, 77, 70, 63, 57, 52], 1.6, ar_coef=0.6, clip=[0,100]),
}

# ── (c) Deadline Miss Rate (%) ─────────────────────────────────────────────────
# Greedy: exponential-like; GA: slower ramp; RL: near-linear then flattening
DMR = {
    "Greedy":      make_curve([ 0.8,  4.2, 10.5, 19.5, 30.0, 40.5, 49.5, 57.0, 63.5, 69.0], 2.5, ar_coef=0.5, clip=[0,100]),
    "GA":          make_curve([ 0.7,  3.2,  8.0, 14.8, 23.0, 32.0, 40.5, 48.0, 54.5, 60.0], 2.2, ar_coef=0.5, clip=[0,100]),
    "Static-PPO":  make_curve([ 0.5,  2.0,  5.0,  9.5, 15.2, 21.8, 28.5, 35.0, 41.0, 46.5], 1.8, ar_coef=0.6, clip=[0,100]),
    "RSDQN":       make_curve([ 0.3,  1.4,  3.5,  6.8, 11.2, 16.5, 22.0, 27.5, 33.0, 38.0], 1.5, ar_coef=0.6, clip=[0,100]),
    "RTGS (Ours)": make_curve([ 0.2,  0.9,  2.4,  5.0,  8.8, 13.2, 17.8, 22.5, 27.5, 32.5], 1.3, ar_coef=0.6, clip=[0,100]),
}

# ── (d) Resource Utilization (%) ───────────────────────────────────────────────
# Greedy: steep drop early; GA: similar; RL methods: more resilient at scale
RU = {
    "Greedy":      make_curve([88, 76, 65, 55, 47, 41, 36, 32, 28, 25], 2.8, ar_coef=0.55, clip=[0,100]),
    "GA":          make_curve([89, 79, 68, 58, 50, 44, 39, 35, 31, 28], 2.5, ar_coef=0.55, clip=[0,100]),
    "Static-PPO":  make_curve([91, 85, 78, 72, 66, 60, 55, 51, 47, 44], 2.0, ar_coef=0.60, clip=[0,100]),
    "RSDQN":       make_curve([92, 87, 82, 77, 72, 67, 62, 58, 55, 51], 1.8, ar_coef=0.60, clip=[0,100]),
    "RTGS (Ours)": make_curve([94, 90, 86, 82, 78, 74, 71, 68, 65, 62], 1.5, ar_coef=0.55, clip=[0,100]),
}
